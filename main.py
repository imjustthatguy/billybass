...
#modified to retrofit billybass.py
#allows for animatronics to respond to alexa voice commands
#by moving their motors when the alexa services are executed

#Overwrite the main.py file in the AlexaPi package with this one
#AlexaPi can be found here
#http://www.github.com/imjustthatguy/AlexaPi
#TODO: Merge main.py code with forked AlexaPi
...
def process_response(response):
	...
	with billybass.BillyBass("reset"):
		print("reset")
...
def trigger_process(trigger):
	...
	if player.is_playing():
		player.stop()
		player.is_playing_speech = False
		player.is_playing_playlist = False
...
if __name__ == "__main__":
	...
	if not silent:
		with billybass.BillyBass("greet"):
	...
	while True:
		time.sleep(.30)
		if player.is_playing():
			if player.is_playing_speech:
				with billybass.BillyBass("response"):
					print("responding")
			elif player.is_playing_playlist:
				time.sleep(.5)
				with billybass.BillyBass("dance"):
					print("dancing")
